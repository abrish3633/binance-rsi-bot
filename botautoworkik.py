...
# Modifications: Removed all MACD code entirely. Trailing stop manually handled by the bot (monitors and updates STOP_MARKET orders via cancel/replace).
# SL/TP placed immediately as fallbacks. Manual trailing: activates at 1.25R, initial stop at initial SL + 0.25R (long)/ -0.25R (short), trails at 2R behind.
# Increased default recvWindow to 30000 for -1021 errors. Telegram notifications for trailing activation/updates.
# Added recovery for SL, TP, and trailing orders (checks if missing/expired and re-places on startup and periodically).
# Other behavior retained. Full ready-to-run script.
...
TRAIL_INITIAL_OFFSET_MULT = Decimal("0.25") # 0.25R above/below initial SL for initial trail stop
VOL_SMA_PERIOD = 15
RSI_PERIOD = 14
...
def trading_loop(client, symbol, timeframe, max_trades_per_day, risk_pct, max_daily_loss_pct, tp_mult, use_trailing, prevent_same_bar, require_no_pos, use_max_loss, use_volume_filter, telegram_bot=None, telegram_chat_id=None):
    ...
    # Startup recovery for all orders
    if has_active_position(client, symbol):
        ...
        if pos_amt != 0:
            ...
            trade_state.initial_sl = Decimal(str(pos.get("entryPrice", "0"))) * (1 - SL_PCT) if trade_state.side == "LONG" else Decimal(str(pos.get("entryPrice", "0"))) * (1 + SL_PCT)
            trade_state.trail_activation_price = Decimal(str(trade_state.entry_price)) * (1 + TRAIL_TRIGGER_MULT * SL_PCT) if trade_state.side == "LONG" else Decimal(str(trade_state.entry_price)) * (1 - TRAIL_TRIGGER_MULT * SL_PCT)
            trade_state.trail_activated = False  # Assume not activated unless known
            trade_state.current_trail_stop = None
            log("Existing position detected on startup. Forcing order recovery for SL/TP/Trailing...", telegram_bot, telegram_chat_id)
            debug_and_recover_expired_orders(client, symbol, trade_state, tick_size, telegram_bot, telegram_chat_id)
