...
def aggregate_klines_to_45m(klines_15m):
    """
    Convert 15m klines → 45m klines.
    Every 3 complete 15m candles → 1 45m candle.
    Keeps data as float (no str conversion).
    """
    if len(klines_15m) < 3:
        return []
    ...
    while i + 2 < len(klines_15m):
        ...
        # Ensure exactly 30 minutes between open and close of group
        if close_time - open_time != 30 * 60 * 1000:
            i += 1  # Skip malformed
            continue
...
def fetch_klines(client, symbol, timeframe, limit=None):
    """
    Fetch klines with 45m support.
    """
    if limit is None:
        limit = max(100, VOL_SMA_PERIOD + 50)
